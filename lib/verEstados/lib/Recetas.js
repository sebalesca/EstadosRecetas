        ...
        if (err) {
            ...
            request.execute('getRecetasObservadas', function(err, recordsets, returnValue) {
                if (err) {
                    ...
                    //IMPORTANTE SELECCIONAR EL PRIMER ELEMNETO DEL ARRAY RECORDSET!!!
                    fs.appendFileSync('logRecetas.txt', 'Cantidad de observadas que se procesaran: ' + _.size(recordsets[0]) + ' \r'); //, function(err) {
        ...
        if (err) {
            ...
            request.execute('getRecetasAutorizadas', function(err, recordsets, returnValue) {
                if (err) {
                    ...
                    //IMPORTANTE SELECCIONAR EL PRIMER ELEMNETO DEL ARRAY RECORDSET!!!
                    fs.appendFileSync('logRecetas.txt', 'Cantidad de Autorizadas que se procesaran: ' + _.size(recordsets[0]) + ' \r'); //, function(err) {
                    console.log(recordsets[0]);
                    callback(null, recordsets[0]);
                }
        ...
        if (err) {
            ...
            request.execute('getRecetasPendientes', function(err, recordsets, returnValue) {
                if (err) {
                    ...
                    //IMPORTANTE SELECCIONAR EL PRIMER ELEMNETO DEL ARRAY RECORDSET!!!
                    fs.appendFileSync('logRecetas.txt', 'Cantidad de pendientes que se procesaran: ' + _.size(recordsets[0]) + ' \r'); //, function(err) {
                    /*    if (err) {
                                            console.log(err);
                                        } else {
...
recetaModelo.verEstadoAces = function(DatosRecetas, callback) {
    ...
    soap.createClient(url, function(err, client) {
        ...
        if (!err) {
            var receta = {};
            receta['Usuario'] = 'usrFarmalive';
            receta['Clave'] = '3rroR@Cam0Yt37f';
...
recetaModelo.procesaEstado = function(DatosReceta, callback) {
    ...
    switch (DatosReceta.Table.idEstado) {
        case '1':
            //validadas pendientes de autorizacion (buzon)
        ...
        case '2':
            ...
            break;
        case '3':
            //Receta previamente procesada por CAMOYTE
            connection = new sql.Connection(config, function(err) {
                // ... err checks
                if (err) {
                    //console.log('no se pudo concectar' + err);
        ...
        case '10':
            ...
            break;
        case '11':
            //11 'Trámite pendiente de validación
            connection = new sql.Connection(config, function(err) {
                // ... err checks
                if (err) {
                    //console.log('no se pudo concectar' + err);
        ...
        case '13':
            ...
            break;
        case '14':
            //En Tramite verificando documentacion afiliado
            connection = new sql.Connection(config, function(err) {
                // ... err checks
                if (err) {
                    //console.log('no se pudo concectar' + err);
        ...
        if (!err) {
            ...
            client.ObtenerRecetasAConsultar(function(err, result) {
                ...
                if (!err) {
                    ...
                    var f2 = new Date();
                    fs.appendFile('./log2.txt', 'FECHA:' + f2.getDate() + "/" + (f2.getMonth() + 1) + "/" + f2.getFullYear() + ' ' + f2.getHours() + ': ' + f2.getMinutes() + ': ' + f2.getMilliseconds() + ' error metodo \r', function(err) {
                        if (err) {
                            console.log(err);
                        } else {
                            console.log('Se ha escrito correctamente el log');
            ...
            fs.appendFile('./log2.txt', 'FECHA:' + f.getDate() + "/" + (f.getMonth() + 1) + "/" + f.getFullYear() + ' ' + f.getHours() + ': ' + f.getMinutes() + ': ' + f.getMilliseconds() + ' error al tratar de Conectar  \r', function(err) {
                if (err) {
                    console.log(err);
                } else {
                    console.log('Se ha escrito correctamente el log');
...
recetaModelo.getRecetasPendientesByFamacia = function(idFarmacia, callback) {
    ...
    var connection = new sql.Connection(config, function(err) {
        ...
        if (err) {
            ...
            //request.verbose = true;
            //request.input('tipo', sql.Char(2), tipo);
            request.input('idFarmacia', sql.Char(9), idFarmacia);
