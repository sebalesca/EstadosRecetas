                           ...
                           console.log('estado igual no se procesa');
                            ...
                            recetaModelo.procesaEstado(recetaAces, function(err) {
                                ...
                                if (!err) {
                                    ...
                                    fs.appendFile('logRecetas.txt', 'idreceta: '+recetaAces.Table.idRecetaFLV + 'error:'+err  +' FECHA:' + f.getDate() + "/" + (f.getMonth() + 1) + "/" + f.getFullYear() + ' ' + f.getHours() + ': ' + f.getMinutes() , function(err) {
                                        if (err) {
                                            console.log(err);
                                        } else {
